...
voronoi_distance_rejection_threshold_squared = 0.01 ** 2 # Meters?. Adjust based on your map resolution and noise level.
pyvoronoi_scaling_factor = 10000
vertex2vertex_merge_threshold_squared = 0.005 ** 2 # Meters?. Adjust based on your map resolution and noise level.
curved_edge_discretization_step = 0.1
douglas_pecker_epsilon = 1.0 # Pixels. Adjust based on your map resolution and desired simplification level.
hierarchy_level = 1
...
class VoronoiDiagramCreator(Node):
    ...
    def map_callback(self, msg):
        ...
        if self.map_x is None:
            ...
            indices = distance_transform_edt(known_locations_mask, return_distances=False, return_indices=True)
        ...
        for i,v in enumerate(vertices):
            for j,vn in enumerate(new_indices):
                if (vertices[j].X-v.X)**2+(vertices[i].Y-v.Y)**2 < vertex2vertex_merge_threshold_squared:
                    new_indices.append(vn)
                    break
            if final_navigable_map.contains(Point(v.X,v.Y)) and vertex_degree[i] > 0:
                node_msg = GraphNode()
            ...
            if i not in removed_edges:
                if e.is_linear:
                    ...
                    for v in pv.DiscretizeCurvedEdge(i,curved_edge_discretization_step):
                        if final_navigable_map.contains(Point(v[0],v[1])):
                            node_msg = GraphNode()
                            # Convert grid coordinates back to real-world meters
